...
//         HAL_UART_Receive_DMA(BspUartUser_HandleTable[uart].uart_handle, BspUartUser_HandleTable[uart].rx_buffer.buffer, 10U);
//     }
...
Bsp_Error_t BspUart_Available(const BspUartUser_Uart_t uart, size_t *const bytes_available)
{
    ...
    if (uart < BSP_UART_USER_MAX)
    {
        ...
        // *bytes_available = (size_t)bytes_received;
        ...
        *bytes_available = (size_t)((write_pointer - BspUartUser_HandleTable[uart].read_pointer) % BspUartUser_HandleTable[uart].rx_buffer_size);
    }
...
//         uint8_t locked = uart->rx_buffer.unlocked ^ 1U;
//         if ((!uart->rx_buffer.reading) && (uart->rx_buffer.read_count[locked] == uart->rx_buffer.write_count[locked]))
//         {
//             Bsp_UartDoubleBuffer_t *double_buffer = &uart->rx_buffer;
